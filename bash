...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .logo-icon { display: inline-flex; align-items: center; justify-content: center; width: 56px; height: 56px; border-radius: 16px; background: linear-gradient(135deg, #25d366 0%, #128c7e 100%); color: #fff; margin-bottom: 16px; }
    ...
    #connect-btn { background: linear-gradient(135deg, #25d366 0%, #128c7e 100%); color: #fff; border: none; border-radius: 10px; padding: 13px; font-size: 15px; font-weight: 600; cursor: pointer; }
    ...
    .info { flex: 1; }
    .settings-btn { background: none; border: 1px solid #1e3524; border-radius: 8px; color: #25d366; font-size: 12px; padding: 6px 10px; cursor: pointer; font-family: inherit; }
    ...
    .bubble { max-width: 72%; padding: 10px 14px; font-size: 14px; line-height: 1.6; color: #d4ead8; }
    ...
    .bubble-label { font-size: 10px; color: #25d366; font-weight: 600; margin-bottom: 4px; letter-spacing: 0.05em; }
    ...
    .error-bar button { background: none; border: none; color: #ff8080; cursor: pointer; font-size: 16px; }
    ...
    #msg-input { flex: 1; background: #0d1710; border: 1px solid #1e3524; border-radius: 10px; padding: 10px 14px; color: #d4ead8; font-size: 14px; resize: none; outline: none; font-family: inherit; line-height: 1.5; min-height: 42px; max-height: 120px; }
    #msg-input:focus { border-color: #25d366; }
    #send-btn { width: 42px; height: 42px; border: none; border-radius: 10px; background: linear-gradient(135deg, #25d366 0%, #128c7e 100%); color: #fff; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
<div id="chat">
  ...
  <div id="error-bar" style="display:none" class="error-bar">
    ...
    <button onclick="document.getElementById('error-bar').style.display='none'">✕</button>
  </div>
  <div class="input-row">
...
<script>
  ...
  function addMsg(role, text, meta) {
    ...
    wrap.innerHTML = `<div class="bubble bubble-${role === "user" ? "user" : "bot"}">${role === "bot" ? '<div class="bubble-label">CLAUDE AI</div>' : ""}<div>${escHtml(text)}</div><div class="bubble-meta">${meta}</div></div>`;
